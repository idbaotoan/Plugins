...
Jobify.App = ( function($) {
	...
	function resizeChosen() {
		...
		jQuery('#search_categories').select2('open');
		jQuery('#search_categories').select2('close');

		// For region
		jQuery('#job_region').select2();
		jQuery('#job_region').select2('open');
		jQuery('#job_region').select2('close');
	}
...
(jQuery), function() {
	...
	function updateContainer() {
		if ( document.getElementById('job-manager-alerts') && this.innerWidth < 768 ) {
			document.getElementById('main').setAttribute('style','flex: 0 0 100%; max-width: 100%' );
			document.getElementById('secondary').setAttribute('style','flex: 0 0 100%; max-width: 100%' );
		} else {
			document.getElementById('main').setAttribute('style','' );
			document.getElementById('secondary').setAttribute('style','' );
		}
	}
